...
export async function GET(req) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Failed to fetch attempted questions" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
...
export async function POST(req) {
  try {
    ...
    return new Response(
      JSON.stringify({ message: "Initialized attempted questions" }),
      {
        status: 201,
        headers: { "Content-Type": "application/json" },
      }
    ...
    return new Response(
      JSON.stringify({ error: "Failed to create attempted questions" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
