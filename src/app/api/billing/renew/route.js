...
export async function POST(req) {
  try {
    ...
    if (!sub) {
      if (!priceId)
        return NextResponse.json({ error: "priceId required to start a new subscription" }, { status: 400 });
    ...
    if (mode === "auto") {
      return NextResponse.json({
        ok: true,
        message: "Auto-renew is handled by Stripe unless cancel_at_period_end is true.",
      });
    }
