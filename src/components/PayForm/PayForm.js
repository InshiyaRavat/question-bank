    ...
    try {
      ...
      if (error) {
        ...
        try {
          ...
          customerId = `temp_${user.id}_${Date.now()}`;
        }
        ...
        if (!subscriptionData.success || !subscriptionData.subscription) {
          ...
          const createRes = await fetch(`/api/subscription`, {
            ...
            body: JSON.stringify({
              ...
              stripeSubscriptionId: `manual_${Date.now()}_${user.id}`,
              stripePriceId: plan == 10 ? "price_6month" : "price_12month",
          ...
          if (!createRes.ok || !createResult.success) {
            console.error("Error while adding new subscriber:", createResult.error);
          ...
          const newPeriodEnd = new Date(newStart);
          newPeriodEnd.setMonth(newPeriodEnd.getMonth() + months);

          const updateRes = await fetch(`/api/subscription/${sub.id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              status: "active",
              duration: months,
              subscribedAt: newStart.toISOString(),
          ...
          if (!updateRes.ok || !updateResult.success) {
            console.error("Error while updating subscription:", updateResult.error);
