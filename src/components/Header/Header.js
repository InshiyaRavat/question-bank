...
const Header = () => {
  ...
  const handleClick = async (e) => {
    ...
    const response = await fetch(`http://localhost:4000/subscription?userid=${user?.id}`);
    ...
    switch (name) {
      ...
      case 'upgrade':
        ...
        const upgraded = new Date(subscription.expiresIn);
        upgraded.setMonth(upgraded.getMonth() + 6);
        await fetch(`http://localhost:4000/subscription/${subscription.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ duration: 12, expiresIn: upgraded.toISOString() }),
        });
        alert('Subscription upgraded!');
        break;
      case 'downgrade':
        if (subscription.duration === 6) return alert('Already at minimum plan.');
        const downgraded = new Date(subscription.expiresIn);
        downgraded.setMonth(downgraded.getMonth() - 6);
        await fetch(`http://localhost:4000/subscription/${subscription.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ duration: 6, expiresIn: downgraded.toISOString() }),
        });
  ...
  return (
    <div className="h-fit lg:h-screen w-full flex">
      ...
      <div className="w-full hidden lg:flex flex-col justify-between bg-white shadow-lg p-6 ">
        <div>
          <h2 className="text-3xl font-bold text-blue-900 mb-6">📚 Question Bank</h2>
          <nav className="flex flex-col gap-4">
            <SidebarButton name="renewal" icon={<RefreshCw />} onClick={handleClick} text="Renew Subscription" color="bg-sky-600" />
            <SidebarButton name="upgrade" icon={<ArrowUpRight />} onClick={handleClick} text="Upgrade Plan" color="bg-emerald-600" />
            <SidebarButton name="downgrade" icon={<ArrowDownLeft />} onClick={handleClick} text="Downgrade Plan" color="bg-yellow-500" />
            <SidebarButton name="cancellation" icon={<XCircle />} onClick={handleClick} text="Cancel Subscription" color="bg-rose-600" />
          </nav>
        </div>
        <UserButton />
      </div>
      ...
      <div className='flex flex-col lg:hidden w-full flex items-center justify-between'>
        <div className="lg:hidden w-full p-4 bg-white shadow flex  items-center justify-between">
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="text-blue-700 p-2 rounded-md hover:bg-blue-100">
